...
import React, { FC, Fragment, memo, MutableRefObject, ReactFragment, useMemo, useRef, useState } from 'react';
...
const FormComposer = (props: formComposerProps) => {
    ...
    const handleValidInputReceived = (property: string) => {
    ...
    const ComposeFormElement = (props: formElementProps) => {
        ...
        if (typeof propertyInfo === "undefined") {
            throw ("Undefined property... is your JSON schema OK?");
        }
        ...
        <AppCard title={<>
            ...
            <AppList>
                ...
                {requiredFields.map(property => {
                    ...
                    {/**Render locked fields as chips */ }
                    if (lockedFields && lockedFields.includes(property))
                        return useMemo(() => <LockedField key={property} property={property} value={instance.current[property]} />, [])
                    {/**don't render hidden fields */ }
                    if (hiddenFields && hiddenFields.includes(property))
                        return useMemo(() => <Fragment key={property}></Fragment>, [])
                    return useMemo(() => (<ComposeFormElement key={property} onValid={handleValidInputReceived} validator={validator} instanceRef={instance} property={property} />), [])
                })}
            </AppList>

            {!requiredOnly && optionalFields.length > 0 && <AppList>
                {/**Compose optional fields */}
                {optionalFields.map(property => {
                    if (lockedFields && lockedFields.includes(property))
                        return useMemo(() => <LockedField property={property} value={instance.current[property]} />, [])
                    if (hiddenFields && hiddenFields.includes(property))
                        return useMemo(() => <Fragment key={property}></Fragment>, [])

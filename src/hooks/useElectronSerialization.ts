...
const useElectronSerialization = create<SynchronizationContext>(() => ({
    async synchronize<T>(serialization: AppSerializationConfig, namespace: string, store: () => Store<T>, uid: string = "", onComplete?: () => void) {
        ...
        const serialized_store_string = await get(collection_key) as string;
        try {
        ...
        try {
            const workspace_string = await get(collection_workspace_key) as string
        ...
        try {
            const active_string = await get(collection_active_key) as string;
